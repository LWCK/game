...
const Step: React.FC = () => {
    ...
    // let stepId = parseInt(param.step) - 1

    // console.log(rule && parse(rule && rule.data.attributes.text));
    // console.log(gameList && gameList.data[stepId].attributes.title);
        ...
        <>
                ...
                <Container className='vh-100 main only-portrait'>
                    <Row className="bg p-4">
                        <Col xs={12} className='text-center '>
                            ...
                            <Button onClick={() => startGame()} variant='' className="btn text step-btn">Commencer <br /> {gameList && gameList.data.attributes.title}</Button>
                        </Col>
                    </Row>
                </Container>
            }
            ...
            {stepGameStory &&
                <Container fluid className='main2 only-portrait bg'>
                    <Row className="p-3">
                        <Col xs={12} className='text text-justify text'>
                            {gameList && parse(gameList.data.attributes.content)}
                ...
                <Container className='only-portrait main bg'>
                    <Row>
                        <Col className='text-center'>
                            <h2>Règles du Jeu</h2>
                            {gameList && parse(gameList.data.attributes.rules_games)}
                            <Image className='game-img' src={'http://192.168.1.250:1337' + gameList.data.attributes.game.data[0].attributes.url} />
                        </Col>
                    </Row>
                </Container>
            }
